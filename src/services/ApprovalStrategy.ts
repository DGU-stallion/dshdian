import { App } from "obsidian";
import { ApprovalLevel } from "../types";
import type { ApprovalDecision } from "../types";
import { getVaultPath } from "../utils";
import * as fs from "fs";
import * as path from "path";

/** Actions that ALWAYS require confirmation regardless of git state */
const ALWAYS_CONFIRM_ACTIONS = ["delete", "remove", "plugin_install", "plugin_uninstall", "system"];

/** Actions considered read-only (always silent) */
const READ_ACTIONS = ["read", "search", "list", "tags", "backlinks", "stat"];

/** Settings interface subset needed by ApprovalStrategy */
interface ApprovalSettings {
	alwaysConfirm: boolean;
	dangerKeywords: string;
	protectedFolders?: string;
}

/**
 * Git-aware approval strategy with overwrite detection and protected folders.
 * Three levels: Silent (read ops), Notify (git + write), Confirm (no git + write, or dangerous ops).
 */
export class ApprovalStrategy {
	private app: App;
	private vaultPath: string;
	private hasGit: boolean | null = null;
	private settings: ApprovalSettings;

	constructor(app: App, settings?: ApprovalSettings) {
		this.app = app;
		this.vaultPath = getVaultPath(app);
		this.settings = settings ?? { alwaysConfirm: false, dangerKeywords: "delete,remove,删除" };
	}

	/** Update settings reference (call when settings change) */
	updateSettings(settings: ApprovalSettings): void {
		this.settings = settings;
	}

	/** Check if vault root has a .git directory */
	async isGitRepo(): Promise<boolean> {
		if (this.hasGit !== null) return this.hasGit;
		const gitDir = path.join(this.vaultPath, ".git");
		return new Promise((resolve) => {
			fs.access(gitDir, fs.constants.F_OK, (err) => {
				this.hasGit = !err;
				resolve(this.hasGit);
			});
		});
	}

	/** Determine approval decision for a given action/tool call */
	async getDecision(action: string, targetPath?: string): Promise<ApprovalDecision> {
		const lower = action.toLowerCase();

		// Settings override: always confirm everything
		if (this.settings.alwaysConfirm) {
			if (!READ_ACTIONS.some((r) => lower.includes(r))) {
				return {
					level: ApprovalLevel.Confirm,
					action,
					description: `Manual confirmation required: ${action}`,
				};
			}
		}

		// Read ops are always silent
		if (READ_ACTIONS.some((r) => lower.includes(r))) {
			return { level: ApprovalLevel.Silent, action, description: action };
		}

		// Check custom danger keywords from settings
		const dangerWords = this.settings.dangerKeywords
			.split(",")
			.map(w => w.trim().toLowerCase())
			.filter(w => w.length > 0);
		if (dangerWords.some(w => lower.includes(w))) {
			return {
				level: ApprovalLevel.Confirm,
				action,
				description: `Dangerous keyword detected: ${action}`,
			};
		}

		// Permanent delete and system ops ALWAYS confirm
		if (ALWAYS_CONFIRM_ACTIONS.some((a) => lower.includes(a))) {
			return {
				level: ApprovalLevel.Confirm,
				action,
				description: `Dangerous operation: ${action}`,
			};
		}

		// Protected folders check
		if (targetPath && this.isProtectedPath(targetPath)) {
			return {
				level: ApprovalLevel.Confirm,
				action,
				description: `Protected folder: ${targetPath}`,
			};
		}

		// Overwrite detection: if write targets an existing file, escalate
		if (targetPath && this.isWriteAction(lower) && this.fileExists(targetPath)) {
			const hasGit = await this.isGitRepo();
			if (!hasGit) {
				return {
					level: ApprovalLevel.Confirm,
					action,
					description: `Overwrite existing file (no git): ${targetPath}`,
				};
			}
			// With git, just notify about overwrite
			return {
				level: ApprovalLevel.Notify,
				action,
				description: `Overwriting: ${targetPath} (git backup available)`,
			};
		}

		// Write operations: check git state
		const hasGit = await this.isGitRepo();
		if (hasGit) {
			// Has git — notify only
			return {
				level: ApprovalLevel.Notify,
				action,
				description: `Executed: ${action}`,
			};
		}

		// No git — all writes require confirmation
		return {
			level: ApprovalLevel.Confirm,
			action,
			description: `No git backup. Confirm: ${action}`,
		};
	}

	/** Reset cached git state (call if vault changes) */
	resetCache(): void {
		this.hasGit = null;
	}

	// ─── Private helpers ───────────────────────────────────────────────

	private isWriteAction(action: string): boolean {
		return ["write", "create", "overwrite", "modify"].some(w => action.includes(w));
	}

	private isProtectedPath(targetPath: string): boolean {
		const protectedStr = this.settings.protectedFolders ?? "";
		if (!protectedStr) return false;
		const folders = protectedStr.split(",").map(f => f.trim()).filter(f => f.length > 0);
		const normalized = targetPath.replace(/\\/g, "/");
		return folders.some(folder => normalized.startsWith(folder + "/") || normalized === folder);
	}

	private fileExists(targetPath: string): boolean {
		const fullPath = path.join(this.vaultPath, targetPath);
		try {
			fs.accessSync(fullPath, fs.constants.F_OK);
			return true;
		} catch {
			return false;
		}
	}
}
